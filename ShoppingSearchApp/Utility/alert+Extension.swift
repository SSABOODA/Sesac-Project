...
extension UIViewController {
    ...
    func showNoQueryAlert() {
        let alert = UIAlertController(title: Constants.AlertText.showNoQueryText, message: nil, preferredStyle: .alert)
        let ok = UIAlertAction(title: Constants.AlertText.ok, style: .default)
        alert.addAction(ok)
    ...
    func showCancelLikeAlert(completionHandler: @escaping () -> ()) {
        ...
        let ok = UIAlertAction(title: Constants.AlertText.ok, style: .default) { _ in
            completionHandler()
        }
        let cancel = UIAlertAction(title: Constants.AlertText.cancel, style: .destructive)
